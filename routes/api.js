...
const listkey = ["Yuibotz", "zeeoneofc"];
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'Yuibotz') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'Yuibotz')  return res.json(loghandler.invalidKey)
...
router.get('/nulis', async (req, res, next) => {
    ...
    if (!text) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter text"})
       ...
       fetch(encodeURI(`https://api.xteam.xyz/magernulis?nama=Rama&kelas=9%20E%202&text=PUNCAKKOMEDI&APIKEY=4ea2f5b23aef889d`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
